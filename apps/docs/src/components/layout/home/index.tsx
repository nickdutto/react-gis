...
import type { BaseLayoutProps, LinkItemType, NavOptions } from "../shared/index";
...
import { getLinks } from "../shared/index";
      ...
      <div className="flex flex-1 flex-row items-center justify-end gap-1.5">
        {searchToggle.enabled !== false &&
          (searchToggle.components?.lg ?? (
            <LargeSearchToggle
              className="w-full max-w-[240px] rounded-full ps-2.5 max-md:hidden"
              hideIfDisabled
            />
          ))}
        {themeSwitch.enabled !== false &&
          (themeSwitch.component ?? (
            <ThemeToggle mode={themeSwitch?.mode} className="max-md:hidden" />
          ))}
        {i18n ? (
          <LanguageToggle>
        ...
        <div className="max-xs:hidden flex flex-row items-center empty:hidden">
